...
FILE_PATH="datasets/edit/loveu/LOVEU-TGVE-2023_Dataset_videovo.csv"
...
do
    ...
    if [ "$video_name" != "Video name" ]; then
        ...
        do
            for ITERS in 3000
            do
                SOURCE_DATA="datasets/edit/loveu/videvo_480p/480p_frames/${video_name}"
                CUDA_VISIBLE_DEVICES=$CUDA python3 main_3dgsvid_full_v2.py -s $SOURCE_DATA \
                    ...
                    --prompt "$style_change" --cate="sty" --progressive_num 2\
                    --cuda_num=$CUDA
                CUDA_VISIBLE_DEVICES=$CUDA python3 main_3dgsvid_full_v2.py -s $SOURCE_DATA \
                    --iteration=$ITERS --use_dual \
                    --group_size=$GNUM --deform_type "multi" \
                    --random_pts_num 60000  --radius 1 3 --biloss_weight 2.0 \
                    --editing_method="progressive_acc" --initial_editor 4 \
                    --prompt "$object_change" --cate="obj" --progressive_num 2\
                    --cuda_num=$CUDA
