...
class Camera(nn.Module):
    def __init__(self, colmap_id, R, T, FoVx, FoVy, image, gt_alpha_mask,
        ...
        self.original_image_nomask = image.clamp(0.0, 1.0).to(self.data_device)

        if gt_detail_image is not None:
            self.detail_image = gt_detail_image.to(self.data_device)
        else:
        ...
        if edited_image is not None:
            if isinstance(edited_image, list):
                self.edited_image = []
                for idx in range(len(edited_image)):
                    self.edited_image.append(edited_image[idx].to(self.data_device))
            else:
