...
def PILtoTorchDepth(pil_image):
    resized_image = torch.from_numpy(np.array(pil_image))
...
def PILtoTorchMask(pil_image, resolution, dilation_size=None):
    resized_image_PIL = pil_image.resize(resolution, Image.NEAREST)
    resized_image_np = np.array(resized_image_PIL)
    resized_image = torch.from_numpy(resized_image_np)
    if dilation_size is not None:
        dilated_mask = dilation(resized_image_np, square(dilation_size))
    ...
    if len(resized_image.shape) == 3:
        ...
        if dilation_size is not None:
            ...
            dilated_mask_image = dilated_mask.unsqueeze(dim=-1).permute(2, 0, 1)
            return resized_image, dilated_mask_image
        else:
            return resized_image.unsqueeze(dim=-1).permute(2, 0, 1)
