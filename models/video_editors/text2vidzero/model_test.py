...
class Model:
    ...
    def  process_pix2pix(self,
        ...
        video_path = video
        video, fps = utils.prepare_vidframes(
            video, resolution, self.device, self.dtype, True, start_t, end_t, out_fps)
        self.generator.manual_seed(seed)
        ...
        # return utils.create_video(result, fps, path=save_path, watermark=gradio_utils.logo_name_to_path(watermark))
        return utils.create_gif_test(result, fps,
            watermark=gradio_utils.logo_name_to_path(watermark),
            scene=scene, prompt=prompt,
            original_res=[480,480], video_path=video_path, dataset=dataset, seed=seed, guide=image_guidance_scale, step=10)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--source', type=str, default="/opt/tiger/inkyunasus/loveu-tgve-2023/gold-fish")
    parser.add_argument('--prompt',
                        type=str,
                        default="Several goldfish swim in a tank.",
                        help='The input path to video.')
    parser.add_argument('--scene', type=str, default="gold-fish")
    ...
    model.process_pix2pix(args.source, args.prompt, args.scene, args.dataset, seed=args.seed, image_guidance_scale=args.guidance)