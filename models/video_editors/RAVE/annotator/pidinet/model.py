...
from models.video_editors.RAVE.annotator.annotator_path import DEVICE
...
def createConvFunc(op_type):
    ...
    if op_type == 'cd':
        ...
        def func(x, weights, bias=None, stride=1, padding=0, dilation=1, groups=1):
            ...
            shape = weights.shape
            if weights.is_cuda:
                buffer = torch.cuda.FloatTensor(shape[0], shape[1], 5 * 5).fill_(0).to(DEVICE)
            else:
                buffer = torch.zeros(shape[0], shape[1], 5 * 5).to(DEVICE)
            weights = weights.view(shape[0], shape[1], -1)
