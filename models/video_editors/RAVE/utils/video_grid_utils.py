...
def prepare_video_to_grid(path, grid_count, grid_size, pad):
    ...
    return grids, frame_count # list of frames



def prepare_videofolder_to_grid(path, grid_count, grid_size, pad):
    ...
    max_grid_area = 512*512* grid_size**2
    # max_grid_area = 256*256* grid_size**2
    grids = []
    frames = []

    total_grid = grid_size**2
    for idx in range(frame_count):
        image = cv.imread(os.path.join(path, file_path[idx]))
        if idx % pad == 0:
            rgb_img = cv.cvtColor(image, cv.COLOR_BGR2RGB)
            rgb_img = np.transpose(rgb_img, (2, 0, 1))
            frames.append(transform(torch.from_numpy(rgb_img)))
